...
기존 레거시 시스템의 '프로모션 상품 목록 조회' 쿼리를 Spring Boot 환경으로 이관하면서 성능 개선을 수행
...
### 5.2 측정 결과
- P-Value: 측정 수치 오름차순 정렬 후의 위치에 해당하는 값
- P50: 100회 측정 중 50위 값 (평소의 상황)
- P95: 100회 측정 중 95위 값 (최악의 상황)
...
> 실제 운영 환경(Oracle, 대용량 데이터)에서는 인덱스 활용 효과로 더 큰 향상을 기대 할 수 있음
> 실제 데이터 수가 천만 이상인 경우는 위 개선 방향과 별도의 처리가 필요 할 수 있음
